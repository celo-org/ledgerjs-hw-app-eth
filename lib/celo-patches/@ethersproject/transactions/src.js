...
const logger = new logger_1.Logger("celo-patched/@ethersproject/transactions");
var TransactionTypes;
(function (TransactionTypes) {
    TransactionTypes[TransactionTypes["legacy"] = 0] = "legacy";
    TransactionTypes[TransactionTypes["eip2930"] = 1] = "eip2930";
    TransactionTypes[TransactionTypes["eip1559"] = 2] = "eip1559";
    TransactionTypes[TransactionTypes["cip64"] = 123] = "cip64";
...
function computeAddress(key) {
    const publicKey = (0, signing_key_1.computePublicKey)(key);
...
function _serializeCip64(transaction, signature) {
    // If there is an explicit gasPrice, make sure it matches the
    // EIP-1559 fees; otherwise they may not understand what they
    // think they are setting in terms of fee.
    if (transaction.gasPrice != null) {
    ...
    const fields = [
        ...
        (transaction.data || "0x"),
        (formatAccessList(transaction.accessList || [])),
        ((transaction.feeCurrency != null) ? (0, address_1.getAddress)(transaction.feeCurrency) : "0x"),
    ];
    if (signature) {
        const sig = (0, bytes_1.splitSignature)(signature);
        fields.push(formatNumber(sig.recoveryParam, "recoveryParam"));
...
function serialize(transaction, signature) {
    ...
    switch (transaction.type) {
        ...
        case 123:
            return _serializeCip64(transaction, signature);
        default:
            break;
    }
...
function _parseEipSignature(tx, fields, serialize) {
    ...
    try {
        const digest = (0, keccak256_1.keccak256)(serialize(tx));
...
function _parseCip64(payload) {
    ...
    tx.hash = (0, keccak256_1.keccak256)(payload);
    _parseEipSignature(tx, transaction.slice(10), _serializeEip1559);
    return tx;
}
...
function _parse(rawTransaction) {
    ...
    try {
        tx.v = bignumber_1.BigNumber.from(transaction[9]).toNumber();
    }
    catch (error) {
        // @TODO: What makes snese to do? The v is too big
        return tx;
    }
    tx.r = (0, bytes_1.hexZeroPad)(transaction[10], 32);
    tx.s = (0, bytes_1.hexZeroPad)(transaction[11], 32);
    if (bignumber_1.BigNumber.from(tx.r).isZero() && bignumber_1.BigNumber.from(tx.s).isZero()) {
        ...
        const digest = (0, keccak256_1.keccak256)(RLP.encode(raw));
        try {
